...
@Repository
public interface DailyPlaceReservationRepository extends JpaRepository<DailyPlaceReservationEntity, Long> {

    // 특정 roomId의 기간별 재고 조회
    List<DailyPlaceReservationEntity> findByRoomIdAndDateBetween(Long roomId, LocalDate startDate, LocalDate endDate);
    ...
    Optional<DailyPlaceReservationEntity> findByRoomIdAndDateForUpdate(@Param("roomId") Long roomId, @Param("date") LocalDate date);

    // 단순 조회용 (락 없이)
    Optional<DailyPlaceReservationEntity> findByRoomIdAndDate(Long roomId, LocalDate date);
