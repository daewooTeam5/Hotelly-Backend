...
public class EmailTemplate {
    public String getCheckInNotifyTemplate(ReservationUser data){
        // 날짜 포맷을 "yyyy년 MM월 dd일"로 변경
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy년 MM월 dd일");
        String checkInDate = data.getResevStart().format(formatter);
        String checkOutDate = data.getResevEnd().format(formatter);
        ...
        return "<!DOCTYPE html>"
                ...
                + "              <h2 style=\"font-size: 24px; font-weight: bold; color: #333333; margin: 0 0 20px 0;\">체크인 알림</h2>"
                + "              <p style=\"font-size: 16px; color: #555555; line-height: 1.6;\">"
                +                 data.getUserName() + "님, 예약하신 호텔의 체크인 날짜가 다가왔습니다.<br>"
                + "                아래 예약 내역을 다시 한번 확인해주세요."
                + "              </p>"
                + "              <table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" style=\"margin: 30px 0;\">"
                + "                <tr>"
                + "                  <td align=\"left\" style=\"background-color: #f1f3f5; border-radius: 8px; padding: 30px 30px;\">" // <!-- 배경색은 인증 코드와 동일하게 -->
                + "                    <p style=\"font-size: 16px; color: #333333; line-height: 1.8; margin: 0 0 15px 0;\">"
                + "                      <strong>호텔:</strong> <span style=\"color: #D4AF37; font-weight: bold;\">" + data.getHotelName() + "</span>" // <!-- 호텔명에 골드 컬러 -->
                + "                    </p>"
                + "                    <p style=\"font-size: 16px; color: #333333; line-height: 1.8; margin: 0 0 15px 0;\">"
                + "                      <strong>객실:</strong> " + data.getRoomName()
                + "                    </p>"
                + "                    <p style=\"font-size: 16px; color: #333333; line-height: 1.8; margin: 0 0 15px 0;\">"
                + "                      <strong>체크인:</strong> " + checkInDate
                + "                    </p>"
                + "                    <p style=\"font-size: 16px; color: #333333; line-height: 1.8; margin: 0;\">"
